...
static void help(const char *progname) {
  ...
  fprintf(stderr, "  -P <Serial Port>      use the specified USB device "
                  "[default: /dev/ttyACM0]\n");
  fprintf(stderr, "  -h                    display this help and exit\n");
  fprintf(stderr,
          "  -o <offset in bytes>  start address for write [default: 0]\n");
  fprintf(stderr, "                        (append 'k' to the argument for "
                  "size in kilobytes,\n");
  fprintf(stderr, "                         or 'M' for size in megabytes)\n");
  fprintf(stderr, "                         or prefix '0x' to the argument for "
                  "size in hexdecimal\n");
  fprintf(stderr, "  --help\n");
...
int main(int argc, char **argv) {
  ...
  if (d == NULL) {
    ...
    strcpy(portName, "/dev/cu.usbmodem000000001");
  } else {
    while ((dir = readdir(d)) !=
  ...
  if (d) // Program Segfaults here on MacOS
    closedir(d);

  /* Decode command line parameters */
  static struct option long_options[] = {{"help", no_argument, NULL, -2},
                                         {NULL, 0, NULL, 0}};
  int opt;
  char *endptr;
  ...
  if (!GetVersion())
    resetFPGA(); // reset the FPGA
  else
    return EXIT_FAILURE;

  int endPage = ((rw_offset + length) >> 16) + 1;

  for (int page = (rw_offset >> 16); page < endPage;
       page++) // erase sufficient 64k sectors
  {
    SerBuf[0] = ERASE_64k;
    SerBuf[1] = page;
    write(fd, SerBuf, 2);
